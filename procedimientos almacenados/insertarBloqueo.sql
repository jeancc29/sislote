...
BEGIN
    ...
    then
		...
		if exists(select id from stocks s where date(s.created_at) = date(now()) and s.idBanca = idBanca and s.idLoteria = idLoteria and s.idLoteriaSuperpale = idLoteriaSuperpale and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 0 and s.idMoneda = @idMoneda)
		then
			set @idStock = (select s.id from stocks s where date(s.created_at) = date(now()) and s.idLoteria = idLoteria and s.idLoteriaSuperpale = idLoteriaSuperpale and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 1 and s.ignorarDemasBloqueos = 1 and s.idMoneda = @idMoneda);
			-- if exists(select id from stocks s where date(s.created_at) = date(now()) and s.idLoteria = idLoteria and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 1 and s.ignorarDemasBloqueos = 1)
			if @idStock is not null
			then 
				-- update stocks s set s.monto = s.monto - @monto where date(s.created_at) = date(now()) and s.idLoteria = idLoteria and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 1 and s.ignorarDemasBloqueos = 1;
			...
			then
				...
				set @idStock = (select s.id from stocks s where date(s.created_at) = date(now()) and s.idBanca = idBanca and s.idLoteria = idLoteria and s.idLoteriaSuperpale = idLoteriaSuperpale and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 0 and s.idMoneda = @idMoneda);
				-- update stocks s set s.monto = s.monto - @monto where date(s.created_at) = date(now()) and s.idBanca = idBanca and s.idLoteria = idLoteria and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 0;
				update stocks s set s.monto = s.monto - @monto where s.id = @idStock;
			end if;
		elseif exists(select s.id from stocks s where date(s.created_at) = date(now()) and s.idLoteria = idLoteria and s.idLoteriaSuperpale = idLoteriaSuperpale and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 1 and s.idMoneda = @idMoneda)
		then
			set @ignorarDemasBloqueos = (select s.ignorarDemasBloqueos from stocks s where date(s.created_at) = date(now()) and s.idLoteria = idLoteria and s.idLoteriaSuperpale = idLoteriaSuperpale and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 1 and s.idMoneda = @idMoneda);
			if @ignorarDemasBloqueos != 1
			then
				set @montoBloqueo = (select b.monto from blocksplays b where date(b.fechaDesde) <= date(now()) and date(b.fechaHasta) >= date(now()) and b.idBanca = idBanca and b.idLoteria = idLoteria and b.jugada = jugada and b.idSorteo = idSorteo and b.status = 1 and b.idMoneda = @idMoneda order by b.id desc limit 1);
				if @montoBloqueo is not null then
					...
					if @montoBloqueo is not null then
						...
						-- update stocks s set s.monto = s.monto - @monto where date(s.created_at) = date(now()) and s.idLoteria = idLoteria and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 1;
						set @idStock = (select s.id from stocks s where date(s.created_at) = date(now()) and s.idLoteria = idLoteria and s.idLoteriaSuperpale = idLoteriaSuperpale and s.jugada = jugada and s.idSorteo = idSorteo and s.esGeneral = 1 and s.idMoneda = @idMoneda);
						update stocks s set s.monto = s.monto - @monto where s.id = @idStock;
					end if;
				end if;
			else
